                        ...
                        const LFSPARMS *lfsparms)
{
   ...
   //Modified by Waqas Ellahi
   int degrees_per_unit = 180 / (float)NUM_DIRECTIONS;
   for (int i = 0; i < minutiae->num; i++){
	   int t = (270 - sround(minutiae->list[i]->direction * degrees_per_unit)) % 360;
	   if (t < 0) {
		   t += 360;
	   }
	   minutiae->list[i]->direction = t;
   }
