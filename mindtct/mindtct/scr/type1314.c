...
***********************************************************************
               ROUTINES:
                        fingerprint2tagged_field_image()
                        image2type_13()
                        image2type_14()
...
/*************************************************************************
**************************************************************************
#cat:   fingerprint2tagged_field_image - Takes a fingerprint image pixmap
...
**************************************************************************/
int fingerprint2tagged_field_image(RECORD **orecord, unsigned char *idata,
                      ...
                      const int img_imp, char *source_str)
{
   ...
   case 3:
      ...
      if((ret = image2type_14(&record, idata, ilen, iw, ih, id,
   ...
   case 7:
      ...
      if((ret = image2type_13(&record, idata, ilen, iw, ih, id,
                             ppmm, img_comp, img_idc, img_imp, source_str))){
         return(ret);
      }
      break;
...
/*************************************************************************
**************************************************************************
#cat:   image2type_13 - Takes a fingerprint image pixmap (possibly
...
#cat:               field image record.
   ...
   Return Code:
      ...
      Negative - system error
**************************************************************************/
int image2type_13(RECORD **orecord, unsigned char *idata, const int ilen,
                  ...
                  const int img_imp, char *source_str)
{
   ...
   if(id != 8){
      fprintf(stderr, "ERROR : image2type_13 : image pixel depth = %d "
   ...
   if(strcmp(img_comp, COMP_NONE) != 0){
      fprintf(stderr, "ERROR : image2type_13 : image compression \"%s\""
   ...
   if((ret = new_ANSI_NIST_record(&record, record_type)))
      return(ret);

   /* 13.001: LEN Field */
   /* Create LEN field with value == "0" (place holder for now). */
   if((ret = value2field(&field, record_type, LEN_ID, "0"))){
   ...
   if((ret = value2field(&field, record_type, IDC_ID, uint_str))){
      free_ANSI_NIST_record(record);
      return(ret);
   }
   ...
   if((ret = value2field(&field, record_type, IMP_ID, uint_str))){
      free_ANSI_NIST_record(record);
      return(ret);
   }
   ...
   if((ret = value2field(&field, record_type, SRC_ID, source_str))){
      free_ANSI_NIST_record(record);
      return(ret);
   }
   ...
   if((ret = get_ANSI_NIST_date(&date_str))){
      free_ANSI_NIST_record(record);
      return(ret);
   }
   /* Create LCD field from date string. */
   if((ret = value2field(&field, record_type, CD_ID, date_str))){
      free(date_str);
      free_ANSI_NIST_record(record);
      return(ret);
   }
   ...
   if((ret = value2field(&field, record_type, HLL_ID, uint_str))){
      free_ANSI_NIST_record(record);
      return(ret);
   }
   ...
   if((ret = value2field(&field, record_type, VLL_ID, uint_str))){
      free_ANSI_NIST_record(record);
      return(ret);
   }
   ...
   if((ret = value2field(&field, record_type, SLC_ID, PP_CM))){
      free_ANSI_NIST_record(record);
      return(ret);
   }
   ...
   if((ret = value2field(&field, record_type, HPS_ID, uint_str))){
      free_ANSI_NIST_record(record);
      return(ret);
   }
   ...
   if((ret = value2field(&field, record_type, VPS_ID, uint_str))){
      free_ANSI_NIST_record(record);
      return(ret);
   }
   /* Append record with HPS field. */
   ...
   if((ret = value2field(&field, record_type, TAG_CA_ID, img_comp))){
      free_ANSI_NIST_record(record);
      return(ret);
   }
   ...
   if((ret = value2field(&field, record_type, BPX_ID, uint_str))){
      free_ANSI_NIST_record(record);
      return(ret);
   }
   ...
   if((ret = value2field(&field, record_type, FGP3_ID, "0"))){
      free_ANSI_NIST_record(record);
      return(ret);
   }
   ...
   if(new_val == NULL){
      fprintf(stderr, "ERROR : image2type_13 : "
   ...
   if((ret = new_ANSI_NIST_field(&field, record_type, DAT2_ID))){
      free_ANSI_NIST_subfield(subfield);
      free_ANSI_NIST_record(record);
      return(ret);
   }
...
/*************************************************************************
**************************************************************************
#cat:   image2type_14 - Takes a fingerprint image pixmap (possibly
...
#cat:               field image record.
   ...
   Return Code:
      ...
      Negative - system error
**************************************************************************/
int image2type_14(RECORD **orecord, unsigned char *idata, const int ilen,
                  const int iw, const int ih, const int id,
                  const double ppmm,  char *img_comp, const int img_idc,
                  const int img_imp, char *source_str)
{
   ...
   /* Only support pixel depth of 8. */
   if(id != 8){
      fprintf(stderr, "ERROR : image2type_14 : image pixel depth = %d "
   ...
   if((strcmp(img_comp, COMP_NONE) != 0) &&
      ...
      fprintf(stderr, "ERROR : image2type_14 : image compression \"%s\""
   ...
   new_val = (unsigned char *)realloc(item->value, ilen);
   if(new_val == NULL){
      fprintf(stderr, "ERROR : image2type_14 : "
	      "realloc : DAT item value (increase %d bytes to %d)\n",
	      item->alloc_chars, ilen);
      free_ANSI_NIST_item(item);
      free_ANSI_NIST_record(record);
